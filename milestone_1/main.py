"""
milestone_1: write basic functions that operate on a list of objects
...
def filter_by_meter_improved(target_meter_id: str, readings_list: list[dict]) -> list[dict]:
    filtered_readings = []
    if not target_meter_id: return filtered_readings
    for reading in readings_list:
        if 'meter_id' not in reading:
            # Ideally we would validate the data before we run it against our business logic functions (filter, total energy, and peak)
            continue
        meter_id = reading['meter_id']
        if meter_id and meter_id == target_meter_id: filtered_readings.append(reading)
    return filtered_readings
